...
import { renderVoiceListingsForPrompt } from './voice-demo-listings';
...
2. Handle the front end of the call yourself — don't offer a transfer just because a question is basic:
   - Property status / availability: check the listings below first — if it's on the list, answer directly using that status/price/size. If it's not on the list, say plainly you don't have that specific one and offer to note it down — but keep going (ask what they're looking for, budget range, timeline) instead of jumping straight to a transfer.
...
export function explainAgentDesign(profile: VoiceAgentProfile): string[] {
  ...
  return [
    ...
    `Sample listings: a handful of fictional Riverside-area properties (with real status, price, size) are loaded in so you can ask "is it still available" and get a direct answer instead of a deflection — swap this for a real MLS/CRM feed once you're live.`,
